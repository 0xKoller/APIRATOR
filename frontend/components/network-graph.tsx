...
function NetworkGraph2D({ targetPerson, contacts, recommended }: NetworkGraphProps) {
  return (
    <div className="w-full h-full p-6 overflow-auto bg-white">
      <div className="mb-4 p-4 border border-gray-200 rounded-lg bg-gray-50">
        <div className="flex items-center gap-4 mb-4">
          <AlertTriangle className="text-amber-500 w-5 h-5" />
          <p className="text-gray-700">3D visualization couldn't be loaded. Showing simplified network view instead.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center mb-8">
        <Avatar className="h-20 w-20 border-2 border-indigo-200 mb-2">
          ...
          <AvatarFallback className="bg-gray-100 text-indigo-600">{targetPerson.name.charAt(0)}</AvatarFallback>
        </Avatar>
        <h3 className="text-lg font-medium text-gray-900">{targetPerson.name}</h3>
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <Avatar className="h-10 w-10 border border-gray-200">
                ...
                <AvatarFallback className="bg-gray-100">{contact.name.charAt(0)}</AvatarFallback>
              </Avatar>
              <div>
                <h4 className="font-medium text-gray-900">{contact.name}</h4>
                <div className="text-xs text-sky-600">{contact.connectionStrength}% match</div>
              </div>
            </div>
            ...
            <div className="text-xs text-gray-600">{contact.mutualConnections} mutual connections</div>
          </motion.div>
        ))}
      </div>
    </div>
